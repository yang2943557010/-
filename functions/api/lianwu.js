...
function buildRawText(url, pwd) {
  ...
  if (!link) return '';
  if (!code) return link;
  if (new RegExp(`[?&](?:pwd|password|code)=${code.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:&|$)`, 'i').test(link)) {
    return link;
  }
...
async function loginLianwu(env) {
  ...
  const password = env?.LIANWU_PASSWORD || 'lianwu-change-me';
...
export async function onRequestPost(context) {
  ...
  if (!auth.ok) {
    const status = /密码|登录|账号/.test(auth.message) ? 401 : 502;
    return jsonResponse({ code: status, message: auth.message || '链坞登录失败', data: null }, status);
  }
